...
function getFriendlyError(err: unknown): string {
  ...
  if (/unauthorized|401/i.test(msg)) return "You must be signed in to run a scan.";
...
const TIERS: TierConfig[] = [
  {
    id: "basic",
    name: "Basic Scan",
    desc: "Core security checks, fast results",
    features: ["Header analysis", "SSL/TLS check", "Tech fingerprint", "OWASP top risks"],
  },
  {
    id: "deep",
    name: "Deep Scan",
    desc: "Full analysis with AI-powered report",
    features: ["Everything in Basic", "AI security analysis", "Remediation guide", "CVE matching"],
    popular: true,
  },
];
          ...
          <div className="pt-6 border-t border-white/5 flex flex-col items-center gap-4">
            ...
            >
              {createScan.isPending ? (
                <>
                  <Loader2 className="w-5 h-5 animate-spin" /> Starting scan…
                </>
              ) : (
                <>
                  <Shield className="w-5 h-5" /> Start Scan — Free
                </>
              )}
            </button>
            ...
            <p className="text-xs text-muted-foreground text-center">
              Scans are free. Only scan websites you own or have explicit permission to test.
            </p>
          </div>
        </form>
      </div>
