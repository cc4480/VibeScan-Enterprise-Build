...
// ── Regression: infrastructure tokens are not credentials ───────────────────
describe("analyzeJwts — infrastructure tokens are not credentials", () => {
  ...
  it("STILL flags a real session token with no expiry", async () => {
    const session = makeJwt({ alg: "HS256", typ: "JWT" }, { sub: "user_8471", email: "a@b.com" });
    const names = (await analyzeJwts({}, `<html>${session}</html>`)).map((f) => f.name).join(" | ");
  ...
  it("STILL flags an authorization token carrying only a role/scope", async () => {
    const svc = makeJwt({ alg: "HS256", typ: "JWT" }, { role: "admin", scope: "read:all" });
    const names = (await analyzeJwts({}, `<html>${svc}</html>`)).map((f) => f.name).join(" ");
    expect(names).toMatch(/No Expiry/i);
  });
});
