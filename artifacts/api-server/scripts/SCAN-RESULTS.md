...
**89 runs across 39 hosts.** Three passive rounds against third-party sites,
plus one active round against a domain we own.

The most recent round is the one to read: **[`scan-results/2026-09-10-rerun/`](scan-results/2026-09-10-rerun/SCAN-RESULTS.md)**
— 38 targets, generated by `render-scan-report.mjs`, with a page per site
carrying every finding and its raw evidence. Seclayer scanned the same 38 the
same night, so the two engines are directly comparable for the first time
(`docs/scan-results/2026-09-10-rerun/` in that repo).
...
## Round 4 — the same 38 targets, both engines, 2026-09-10

The union of the round 1 and round 2 lists, re-scanned on current code. Full
tables and per-site pages: [`scan-results/2026-09-10-rerun/`](scan-results/2026-09-10-rerun/SCAN-RESULTS.md).

**30 sites graded:** A 3 · B 15 · C 11 · D 1 · F 0 — mean score 22.3, against
22.2 in round 1 and 22.1 in round 2 over their own graded sets. Three rounds,
two days apart, across a scanner change, landing within 0.2 of each other is the
result that matters here: the numbers are reproducible.

6 sites were intercepted and are ungraded. 2 were unreachable — `fastly.com` and
`mozilla.org` both returned `fetch failed`, transient and unrelated to the
scanner; both scanned fine in earlier rounds.

### What this round found

**An intercepted scan is handed a perfect grade by the product.**

The report tables here withhold a score for those sites, because
`render-scan-report.mjs` refuses to grade them. `computeRiskScore` does not:
its input is the withheld finding set, which is entirely info, which weighs 0.

```
apnews.com         computeRiskScore = 0  → grade A
stackoverflow.com  computeRiskScore = 0  → grade A
amazon.co.uk       computeRiskScore = 0  → grade A
ebay.com           computeRiskScore = 0  → grade A
reuters.com        computeRiskScore = 0  → grade A
etsy.com           computeRiskScore = 1  → grade A
```

Zero is the best score on the scale. So the scan tells the customer, in the
finding text, that this is *not* a clean result — and then grades it better than
every site it actually managed to read.

Seclayer has the identical defect from the opposite end: `scoreFindings` over
the withheld set returns **100/A**, its top grade, for all four sites it was
blocked on.

This is the audit's recurring error wearing new clothes. Every previous instance
attributed the interstitial's *faults* to the customer; this one attributes its
*silence* to them as merit. Both come from treating a response the target never
sent as evidence about the target.

Not yet fixed in either product — it is a scoring-semantics decision, not a
patch.
